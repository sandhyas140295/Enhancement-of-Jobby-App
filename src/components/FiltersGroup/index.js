...
const FiltersGroup = props => {
  ...
  const renderTypesOfLocation = () => {
    const {updatedLocationTypes} = props
    ...
    return locations.map(eachType => (
      <li className="fliters-list-item" key={eachType.locationId}>
        <input
          ...
          onChange={() => updatedLocationTypes(eachType.label)}
        />
  ...
  const renderLocationTypes = () => (
    <>
      ...
      <ul className="filters-list">{renderTypesOfLocation()}</ul>
    </>
  )
