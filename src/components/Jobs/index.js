...
class Jobs extends Component {
  ...
  updatedLocationTypes = id => {
    const {locations} = this.state

    let updatedList = locations
    if (locations.includes(id)) {
      updatedList = locations.filter(eachType => eachType !== id)
    } else {
      updatedList = [...updatedList, id]
    }
    this.setState({locations: updatedList}, this.getJobs)
  }
  ...
  filteredJobList = () => {
    ...
    if (locations.length > 0) {
      updatedList = jobsList.filter(each => {
    ...
    return (
      <div className="side-bar">
        ...
        <FiltersGroup
          ...
          updatedLocationTypes={this.updatedLocationTypes}
          locations={locations}
        />
      </div>
    )
